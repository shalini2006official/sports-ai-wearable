...
import {
  ...
  BarChart3, User, Eye, EyeOff, Trophy,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        setData(shaped);
        setSource(json.sources?.wrist === "live" ? "live" : "simulated");
    ...
    <div className="flex flex-col items-center justify-center">
      <div style={{ position: "relative", width: size, height: size }}>
        ...
        <div style={{ position: "absolute", inset: 0 }} className="flex items-center justify-center f-mono font-semibold" >
          <span style={{ color: C.text, fontSize: size * 0.22 }}>{Math.round(value)}</span>
        </div>
      </div>
    ...
    <div className="min-h-screen flex f-body" style={{ background: C.void, color: C.text }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="px-6 pt-4">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] f-mono"
            style={{ background: source === "live" ? "rgba(52,211,153,0.12)" : "rgba(251,191,36,0.12)", color: source === "live" ? C.green : C.amber }}>
            <span className="w-1.5 h-1.5 rounded-full pulse-dot" style={{ background: source === "live" ? C.green : C.amber }} />
            {source === "live" ? "LIVE — reading from ESP32 backend" : "SIMULATED — backend not reachable, using demo data"}
          </span>
        </div>
          ...
          {active === "overview" && (
            <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <GlassCard className="p-4 flex flex-col items-center"><Ring value={(t.hr / 190) * 100} color={C.pulse} label="Heart Rate" sub={`${t.hr} BPM`} /></GlassCard>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <GlassCard className="p-5">
                  ...
                  <div className="space-y-2.5 f-body text-sm">
                    <div className="flex justify-between"><span style={{ color: C.muted }}>ESP32</span><ConnDot connected /></div>
                    <div className="flex justify-between"><span style={{ color: C.muted }}>MPU6050</span><ConnDot connected /></div>
                    <div className="flex justify-between"><span style={{ color: C.muted }}>MAX30102</span><ConnDot connected /></div>
                    <div className="flex justify-between"><span style={{ color: C.muted }}>Last Updated</span><span className="f-mono text-xs">just now</span></div>
                  </div>
                </GlassCard>
              </div>
                  ...
                  <table className="w-full text-sm f-body">
                    <thead>
                      <tr style={{ color: C.muted, borderBottom: `1px solid ${C.border}` }}>
                        {["Date", "Duration", "Avg HR", "Avg SpO₂", "Score", "Type"].map((h) => <th key={h} className="text-left py-2 font-medium text-xs">{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {SESSIONS.map((s, i) => (
                        <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                          ...
                          <td className="f-mono">{s.duration}</td>
                          <td className="f-mono">{s.avgHr} BPM</td>
                          <td className="f-mono">{s.avgSpo2}%</td>
                          <td className="f-mono" style={{ color: C.green }}>{s.score}</td>
                          <td>{s.activity}</td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="space-y-3 max-w-2xl">
              {messages.map((m, i) => (
                <GlassCard key={i} className="p-4 flex gap-3 items-start">
                  <div className="p-2 rounded-lg flex-shrink-0" style={{ background: "rgba(255,255,255,0.05)" }}><m.icon size={16} color={C.text} /></div>
                  <div className="flex-1">
                    <PriorityBadge p={m.p} />
                    <p className="f-body text-sm mt-2" style={{ color: C.text }}>{m.text}</p>
                  </div>
                </GlassCard>
              ))}
            </div>
...
export default function App() {
  const [view, setView] = useState("landing"); // landing | login | player | coach
  ...
  const handleLogin = (role, email) => {
    setSession({ role, name: role === "player" ? "Alex Carter" : "Coach Delgado" });
    setView(role === "player" ? "player" : "coach");
  };
  ...
  if (view === "player") return <PlayerDashboard tick={tick} onLogout={handleLogout} name={session?.name} />;
  if (view === "coach") return <CoachDashboard tick={tick} onLogout={handleLogout} name={session?.name} />;
  return null;
}